...
public interface GameApiAware {
    ...
    default List<UndoNStepPath> undo(TestRestTemplate template, String game, String car, int moves) {
        String url = String.join("/", "/v1/back-movements", game, car, moves + "");

        ResponseEntity<UndoNStepPath[]> responseEntity = template.getForEntity(url, UndoNStepPath[].class);
        return Arrays.asList(responseEntity.getBody());
    }
